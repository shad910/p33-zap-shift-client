...
1. **Pro-Fast** is a home/office pickup parcel delivery system that streamlines booking, tracking, and delivery processes.
...
1. The **Pro-Fast** system is designed around three core user roles — **User**, **Admin**, and **Rider** — each responsible for distinct stages in the parcel delivery process.
...
4. Show Buttons & Cards based on status with a Ordered-List
   ...
   2. **Parcel received by Rider Card** (Show it when status == ready-to-pickup)
   3. **Confirm Parcel Received** (show it if service centers are different & enable it when status == in-transit)
   ...
   7. **Parcel Delivery Successful Card** (Show it when status == delivered)

5. **Button Functionalities**

   1. **Assign Parcel for PickUp**:

      1. Show a Modal with pickup service center Info, Rider who works in the service center
   ...
   4. **Assign Parcel for Delivery**
      ...
      1. Show a Modal with delivery service center Info, Rider who works in the service center
      2. On Select a Rider Card Show Assign Button
