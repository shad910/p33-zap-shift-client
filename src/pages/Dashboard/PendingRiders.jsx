...
const PendingRiders = () => {
    ...
    const { data: riders = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            const response = await useAxios.get("/riders/pending");
    ...
    const approveMutation = useMutation({
        mutationFn: async (id) => {
            const res = await fetch(`http://localhost:5000/riders/${id}/approve`, { method: "PATCH" });
            return res.json();
        },
        onSuccess: (data) => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id) => {
            const res = await fetch(`http://localhost:5000/riders/${id}`, { method: "DELETE" });
            return res.json();
        },
        onSuccess: (data) => {
    ...
    const handleApprove = (id) => {
        Swal.fire({
            title: "Are you sure?",
            text: "This rider will be approved.",
            icon: "warning",
                            ...
                            {displayRiders.length > 0 ? (
                                displayRiders.map((rider) => (
                                    <tr key={rider._id}>
                                        ...
                                        <td>{new Date(rider.created_at).toLocaleString()}</td> {/* Applied */}
                ...
                <div className="md:hidden space-y-4 p-2">
                    {displayRiders.length > 0 ? displayRiders.map((rider) => (
                        <div key={rider._id} className="border rounded-lg p-2">
                            ...
                            <p><strong>Applied:</strong> {new Date(rider.created_at).toLocaleString()}</p> {/* Applied */}
                            <p><strong>Status:</strong> <span className="badge badge-xs badge-warning capitalize">{rider.status}</span></p>
                ...
                <div className="modal-box max-w-2xl">
                    ...
                    {selectedRider && (
                        <div className="space-y-2 text-sm">
                            ...
                            <p><strong>Applied:</strong> {new Date(selectedRider.created_at).toLocaleString()}</p> {/* Applied */}
                        </div>
                    )}
